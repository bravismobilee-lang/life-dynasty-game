const KEY='life-dynasty-v3';const ranks=[['БРОДЯГА',0],['РАБОЧИЙ',25],['СПЕЦИАЛИСТ',100],['ИНВЕСТОР',400],['МИЛЛИОНЕР',1500],['ЛЕГЕНДА',5000]];const gear=['🔩','🧲','📟','📼','💾','🔮','💎','🧿'];const gearName=['Обломок','Магнит','Пейджер','Кассета','Модуль','Ядро','Кристалл','Реликвия'];let S=JSON.parse(localStorage.getItem(KEY)||'{"ldn":0,"silver":120,"energy":100,"items":[1,1,2],"income":0,"eventDone":false,"taps":0}');S.taps=S.taps||0;if(!localStorage.getItem('demo-silver-10000-v1')){S.silver=10000;localStorage.setItem('demo-silver-10000-v1','1');localStorage.setItem(KEY,JSON.stringify(S));}S.selected=null;S.sellMode=false;S.queue=S.queue||[];if(!Array.isArray(S.board)){S.board=[]}if(S.board.length!==30){let old=S.board.filter(Boolean);S.board=Array(30).fill(null);old.slice(0,30).forEach((v,i)=>S.board[i]=v);if(!old.length)(S.items||[1,1,2]).slice(0,30).forEach((v,i)=>S.board[i]=v);localStorage.setItem(KEY,JSON.stringify(S))}const one=q=>document.querySelector(q);const all=q=>Array.from(document.querySelectorAll(q));function tapCost(){return 0.1}function rank(){let r=0;ranks.forEach((x,i)=>{if(S.ldn>=x[1])r=i});return r}function save(){localStorage.setItem(KEY,JSON.stringify(S));render()}function toast(x){one('#toast').textContent=x;one('#toast').classList.add('show');setTimeout(()=>one('#toast').classList.remove('show'),800)}const heroCards=[{rarity:'COMMON',sex:'♂',name:'Бродяга',icon:'🧔🏻',pet:'🐕'},{rarity:'COMMON',sex:'♀',name:'Бродяжка',icon:'👩🏻',pet:'🐈'},{rarity:'RARE',sex:'♂',name:'Ночной бродяга',icon:'🧔🏻‍♂️',pet:'🐦'},{rarity:'RARE',sex:'♀',name:'Уличная лиса',icon:'👩🏻‍🦱',pet:'🦊'},{rarity:'EPIC',sex:'♂',name:'Теневой странник',icon:'🥷',pet:'🐺'},{rarity:'EPIC',sex:'♀',name:'Неоновая странница',icon:'👩🏻‍🎤',pet:'🐈‍⬛'},{rarity:'LEGENDARY',sex:'♂',name:'Король улиц',icon:'🤴🏻',pet:'🦁'},{rarity:'LEGENDARY',sex:'♀',name:'Королева улиц',icon:'👸🏻',pet:'🦅'},{rarity:'MYTHIC',sex:'♂',name:'Легенда',icon:'🧙🏻‍♂️',pet:'🐉'},{rarity:'MYTHIC',sex:'♀',name:'Легенда',icon:'🧙🏻‍♀️',pet:'🐉'}];let heroIndex=+(localStorage.getItem('hero-demo-index')||0);function renderHeroDeck(){let tr=one('#heroTrack'),dots=one('#heroDots');if(!tr||!dots)return;tr.innerHTML=heroCards.map((c,i)=>'<article class="heroCard '+c.rarity.toLowerCase()+'"><div class="rarity">'+c.rarity+'</div><div class="sex">'+c.sex+'</div><div class="heroPortrait"><span>'+c.icon+'</span><i>'+c.pet+'</i></div><div class="heroName">'+c.name+'</div><small>LV 1</small></article>').join('');dots.innerHTML=heroCards.map((_,i)=>'<i class="'+(i===heroIndex?'on':'')+'"></i>').join('');tr.style.transform='translate3d('+(-heroIndex*100)+'%,0,0)';let sx=0,dx=0,drag=false;tr.onpointerdown=e=>{drag=true;sx=e.clientX;dx=0;try{tr.setPointerCapture(e.pointerId)}catch(_){}};tr.onpointermove=e=>{if(!drag)return;dx=e.clientX-sx;tr.style.transition='none';tr.style.transform='translate3d(calc('+(-heroIndex*100)+'% + '+dx+'px),0,0)'};const end=()=>{if(!drag)return;drag=false;if(Math.abs(dx)>45)heroIndex=Math.max(0,Math.min(heroCards.length-1,heroIndex+(dx<0?1:-1)));localStorage.setItem('hero-demo-index',heroIndex);tr.style.transition='transform .28s cubic-bezier(.2,.8,.2,1)';tr.style.transform='translate3d('+(-heroIndex*100)+'%,0,0)';all('#heroDots i').forEach((d,i)=>d.classList.toggle('on',i===heroIndex));dx=0};tr.onpointerup=end;tr.onpointercancel=end}setTimeout(renderHeroDeck,0);
