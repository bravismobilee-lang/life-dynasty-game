...
function mergeTap(i){let l=S.items[i];if(S.sellMode){let v=Math.pow(3,l-1)*8;S.items.splice(i,1);S.silver+=v;S.sellMode=false;S.selected=null;toast('Продано: +'+v+' серебра');return save()}if(S.selected===null){S.selected=i;return render()}if(S.selected===i){S.selected=null;return render()}let a=S.selected;if(S.items[a]===l&&l<5){let hi=Math.max(a,i),lo=Math.min(a,i);S.items.splice(hi,1);S.items.splice(lo,1);S.items.push(l+1);S.selected=null;toast('MERGE! Получен LV '+(l+1));return save()}S.selected=i;render()}if(one('#sellMode'))if(one('#sellMode'))one('#sellMode').onclick=()=>{S.sellMode=!S.sellMode;S.selected=null;one('#sellMode').classList.toggle('active',S.sellMode);toast(S.sellMode?'Выбери предмет для продажи':'Продажа отменена')};function upgrade(i){let x=[[50,.2],[150,.7],[500,2]][i];if(S.silver<x[0])return toast('Не хватает серебра');S.silver-=x[0];S.income+=x[1];save();toast('Доход вырос')}const demoDynasty=[{lv:1,sex:'♂',name:'Бродяга',face:'🧔🏻',xp:100},{lv:1,sex:'♀',name:'Бродяга',face:'👩🏻',xp:100},{lv:2,sex:'♂',name:'Рабочий',face:'👨🏻‍🔧',xp:220},{lv:2,sex:'♀',name:'Рабочая',face:'👩🏻‍🔧',xp:220},{lv:3,sex:'♂',name:'Специалист',face:'👨🏻‍💼',xp:460},{lv:4,sex:'♀',name:'Инвестор',face:'👩🏻‍💼',xp:900},{lv:5,sex:'♂',name:'Династия',face:'🧔🏻‍♂️',xp:1800}];function renderDynastyCards(){let box=one('#dynastyCards');if(!box)return;box.innerHTML=demoDynasty.map((c,i)=>'<button class="dynastyCard lv'+c.lv+'" data-dcard="'+i+'"><i>'+c.sex+'</i><em>'+c.face+'</em><small>LV '+c.lv+'</small><b>'+c.name+'</b><span>'+c.xp+' / '+c.xp+'</span></button>').join('');all('[data-dcard]').forEach(b=>b.onclick=()=>{let c=demoDynasty[+b.dataset.dcard];toast(c.sex+' '+c.name+' · LV '+c.lv)})}setTimeout(renderDynastyCards,0);if(one('#breedDemo'))one('#breedDemo').onclick=()=>{let btn=one('#breedDemo');btn.textContent='♂ LV1  +  ♀ LV1  →  LV2 ✦';btn.classList.add('bred');setTimeout(()=>{btn.textContent='ПОКАЗАТЬ СКРЕЩИВАНИЕ';btn.classList.remove('bred')},1800)};
